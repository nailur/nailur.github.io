...
function handleSessionSync(session) {
...
async function fetchMarketData() {
    ...
    const brands = new Set();
    const uniqueMap = new Map();   // Menyimpan data harga Hari Ini (Today) dari API
    const historyMap = new Map();  // Menyimpan data harga Kemarin/Sebelumnya (Yesterday) dari DB
    
    brandWeightMap = {};

    // 3. Ekstrak data dari DB Supabase terlebih dahulu untuk data pembanding (Yesterday) & fallback
    if (dbData) {
        dbData.forEach(item => {
    ...
    // 4. Bangun atau timpa data Hari Ini (uniqueMap) murni menggunakan format JSON API baru (Physical Only)
    if (apiData && Array.isArray(apiData)) {
        // Kosongkan map utama agar tampilan hari ini murni dari data API fresh
        uniqueMap.clear();
        ...
        apiData.forEach(apiItem => {
            ...
            brands.add(fallbackBrandId + "|" + brandName);
            
            if (!brandWeightMap[fallbackBrandId]) brandWeightMap[fallbackBrandId] = new Set();
            brandWeightMap[fallbackBrandId].add(weight);
        });
    }
